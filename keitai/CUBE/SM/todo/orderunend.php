<?php
if($_GET["ordersort"]!=NULL) {
	$_SESSION["ordertodo"]["sort"]=$_GET["ordersort"];
}
if($_GET["pmode"]=="logdel") {
	...
	location.replace("?PID=todo_orderunendbox");
	</script>
	<?php
}

if($_SESSION["ordertodo"]["sort"]=="all"||$_SESSION["ordertodo"]["sort"]==NULL) {
...
$sql="select * from todo inner join member on member.member_id = todo.to_member_id where from_member_id = ".$logindata["member_id"]."  and to_member_id<>".$logindata["member_id"]." "."and status = ".$_SESSION["ordertodo"]["sort"]." order by status,hopedate,id desc";
...
function delchk(id) {
	...
	if(res) {
		location.replace("?PID=todo_orderunendbox&pmode=logdel&delid="+id);
	}
}
//-->
</script>
                                ...
                                <td>
                                    <div align="right">
                                        <select name="mysort" id="mysort" onChange="MM_jumpMenu('parent',this,0)">
                                            <option value="?PID=todo_orderunendbox&ordersort=1"<?php if($_SESSION["ordertodo"]["sort"]==1){echo " selected";}?>>未確認</option>
                                            <option value="?PID=todo_orderunendbox&ordersort=2"<?php if($_SESSION["ordertodo"]["sort"]==2){echo " selected";}?>>確認済</option>
                                            <option value="?PID=todo_orderunendbox&ordersort=5"<?php if($_SESSION["ordertodo"]["sort"]==5){echo " selected";}?>>作業中</option>
                                            <option value="?PID=todo_orderunendbox&ordersort=all"<?php if($_SESSION["ordertodo"]["sort"]=="all"){echo " selected";}?>>全て表示</option>
                                        </select>
                                  </div>
                                </td>
                                        </form>
                    ...
                    <td valign="top" bgcolor="#FFFFFF">
                        <table width="100%" border="0" cellpadding="5" cellspacing="0">
                            <tr>
                                ...
                                <td width="19%" nowrap bgcolor="#ececec">
                                    <p>依頼先</p>                                </td>
                                <td width="20%" nowrap="nowrap" bgcolor="#ececec">終了予定</td>
                                <td width="20%" nowrap bgcolor="#ececec">依頼期限</td>
                                <td width="6%" nowrap bgcolor="#ececec">
                                    <div align="left">状態</div>
                                    <div align="center"></div>                                </td>
                                <td width="6%" nowrap bgcolor="#ececec"> 
                                    <div align="center">編集</div>                                </td>
                                <td width="11%" nowrap bgcolor="#ececec">
...
?></td>
							  ...
							  echo str_replace("-",".",str_replace("00:00:00","",$todolist[$i]["hopedate"]));
                                ...
                                <td bgcolor="#FFFFFF">
                                    <div align="center"><a href="?PID=todo_up&id=<?php echo $todolist[$i]["id"];?>"><img src="img/template/bbs/edit.gif" width="15" height="16" border="0"></a></div>                                </td>
                                <td bgcolor="#FFFFFF">
                            ...
                            <tr>
                                <td>依頼先 　
                                    <?php
																																...
																																?>
                                    <span class="comment">　 <?php echo str_replace("-",".",$meslist[$j]["registdate"]);?></span></td>
                            </tr>
