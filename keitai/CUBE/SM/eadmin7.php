...
# 2007.10.12 v.2.00.02 固定絵文字一覧表示絵文字No表示化
# 2007.12.25 v.2.00.03 DoCoMo絵文字ﾃｷｽﾄｺｰﾄﾞ指定追加
###############################################################################
$ver = 'v.2.00.03';
###############################################################################
...
function set_vew() {
  ...
  $emoji_non_1 = '';
  $emoji_non_2 = '';
  if (isset($GLOBALS['emoji_obj']->emoji_non)) {
    ${'emoji_non_'.$GLOBALS['emoji_obj']->emoji_non} = 'selected';
  }
  $emojiset_DoCoMo   = '';
  $emojiset_au       = '';
  $emojiset_SoftBank = '';
  if (isset($GLOBALS['emoji_obj']->emojiset)) {
        ...
        <tr valign=\"top\">
          ...
          <td>
            <font style=\"font-size:10px; color:#ff0000\">
              メインスクリプトからの相対パス、或いはサーバー上での絶対パスで指定してください。<br>
            </font>
          </td>
        </tr>
        <tr valign=\"top\">
    ...
    <td bgcolor=\"#f0f0ff\" style=\"font-size:10px\" valign=\"top\">
      ...
      <table bgcolor=\"\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\" align=\"center\" width=\"100%\">
        ...
        <tr valign=\"top\">
          ...
          <td>
            <font style=\"font-size:10px; color:#ff0000\">
              下位の'/docomo'や'/au'ディレクトリまでの指定は不要です。<br>
            </font>
          </td>
        </tr>
        <tr valign=\"top\">
  ...
  <tr style=\"font-size:12px\">
    ...
    <td bgcolor=\"#0000ff\"><font style=\"font-weight:bold; color:#ffffff\">文字コード指定<br></font></td>
    <td bgcolor=\"#f0f0ff\">
    ...
    <td bgcolor=\"#f0f0ff\" style=\"font-size:10px\" valign=\"top\">
      デフォルトの文字コードを指定します。<br>
      メインスクリプトの文字コードや、扱う文字データのコードに合わせることを推奨致します。<br>
      主に出力文字コードに影響します。<br>
    </td>
  </tr>
  <tr style=\"font-size:12px\">
    <td bgcolor=\"#0000ff\" align=\"right\"><font color=\"#ffffff\">08<br></td>
    <td bgcolor=\"#0000ff\"><font style=\"font-weight:bold; color:#ffffff\">固定絵文字指定<br></font></td>
    <td bgcolor=\"#f0f0ff\">
      ■PC表示時絵文字キャリア指定<br>
      ...
      ■DoCoMo絵文字テキストコード指定<br>
      <select name=\"docomo_fix_code\">
        <option value=\"\">▼選択</option>
        <option value=\"Unicode\" ".$docomo_fix_code_Unicode.">Unicode</option>
        <option value=\"SJIS\" ".$docomo_fix_code_SJIS.">Shift-JIS</option>
      </select>
    </td>
    <td bgcolor=\"#f0f0ff\" style=\"font-size:10px\" valign=\"top\">
      固定絵文字表示について指定します。<br>
      PC表示時での表示絵文字キャリアを指定します。<br>
      また、DoCoMo携帯でのテキストコードを指定します。
      DoCoMoではUnicodeを推奨していますが、mova等では表示できない場合があります。
      Shift-JISコードでは全ての携帯で表示きますが、拡張絵文字に対応していません。<br>
  ...
  <tr style=\"font-size:12px\">
    ...
    <td bgcolor=\"#0000ff\"><font style=\"font-weight:bold; color:#ffffff\">文字コード認識順位設定<br></font></td>
    <td bgcolor=\"#f0f0ff\">
      ■Shift_JISコード用設定：<br>
      <input type=\"text\" size=\"50\" name=\"encode_list_sjis\" value=\"".$GLOBALS['emoji_obj']->encode_list_sjis."\">
      ■EUC-JPコード用設定：<br>
      <input type=\"text\" size=\"50\" name=\"encode_list_euc\" value=\"".$GLOBALS['emoji_obj']->encode_list_euc."\">
      ■UTF-8コード用設定：<br>
      <input type=\"text\" size=\"50\" name=\"encode_list_utf8\" value=\"".$GLOBALS['emoji_obj']->encode_list_utf8."\">
    </td>
    <td bgcolor=\"#f0f0ff\" style=\"font-size:10px\" valign=\"top\">
      mb_detect_encoding関数においての文字コード認識順位を指定します。<br>
      コード変換等において文字化けが発生する様であれば任意に設定してください。<br>
      <table bgcolor=\"\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\" align=\"center\" width=\"100%\">
        <tr valign=\"top\">
          <td width=\"1pt\"><font style=\"font-size:10px; color:#ff0000\">※<br></font></td>
          <td>
            <font style=\"font-size:10px; color:#ff0000\">
              各文字コードはカンマで区切りで指定し、リスト左側が優先されます。<br>
            </font>
          </td>
        </tr>
      </table>
...
#    <td bgcolor=\"#0000ff\" align=\"right\"><font color=\"#ffffff\">20<br></td>
...
function set_save() {
  ...
  if ($_REQUEST['docomo_fix_code'] == '') { err('固定絵文字(DoCoMo用テキストパターン)指定が指定されていません。'); }
  # DoCoMo絵文字ｶﾗｰ化設定ﾁｪｯｸ
...
#    err('携帯画像変換スクリプトの設置場所が指定されていません。');
  ...
  # 文字ｺｰﾄﾞ認識順位設定ﾁｪｯｸ
  if ($_REQUEST['encode_list_sjis'] == '') { $_REQUEST['encode_list_sjis'] = 'SJIS-win,SJIS,JIS,eucJP-win,EUC-JP,UTF-8'; }
  if ($_REQUEST['encode_list_euc'] == '')  { $_REQUEST['encode_list_euc']  = 'eucJP-win,EUC-JP,SJIS-win,SJIS,JIS,UTF-8'; }
  if ($_REQUEST['encode_list_utf8'] == '') { $_REQUEST['encode_list_utf8'] = 'UTF-8,SJIS-win,SJIS,JIS,eucJP-win,EUC-JP'; }
...
#  }
  ...
  $GLOBALS['SETTING_DATA'][] = "encode_list_sjis\t".$_REQUEST['encode_list_sjis']."\t";
  $GLOBALS['SETTING_DATA'][] = "encode_list_euc\t".$_REQUEST['encode_list_euc']."\t";
  $GLOBALS['SETTING_DATA'][] = "encode_list_utf8\t".$_REQUEST['encode_list_utf8']."\t";
  $GLOBALS['SETTING_DATA'][] = "docomo_fix_code\t".$_REQUEST['docomo_fix_code']."\t";
...
";
  ...
  if ($GLOBALS['emoji_obj']->db_flag != '1') {
    ...
    <td>
      <i>\$obj-&gt;</i><b>FIX_EMJ['".$enob."']</b><br>
    </td>\n";
      if (isset($DDATA_D0[$d_nob])) {
        if ($DDATA_D0[$d_nob] != '') {
          $print_data .= "    <td><img src=\"".$GLOBALS['emoji_obj']->emjimg_path."/".$DDATA_D0[$d_nob]."\" border=\"0\"> ".$DDATA_D1[$d_nob]."<br><font style=\"font-size:8pt\">(No.".$d_nob.")</font><br></td>\n";
        } else {
      ...
      if (isset($DDATA_V0[$v_nob])) {
        if ($DDATA_V0[$v_nob] != '') {
          $print_data .= "    <td><img src=\"".$GLOBALS['emoji_obj']->emjimg_path."/".$DDATA_V0[$v_nob]."\" border=\"0\"> ".$DDATA_V1[$v_nob]."<br><font style=\"font-size:8pt\">(No.".$v_nob.")</font><br></td>\n";
        } else {
          $print_data .= "    <td align=\"center\">-<br></td>\n";
        }
      } else {
      ...
      if (isset($DDATA_A0[$a_nob])) {
        if ($DDATA_A0[$a_nob] != '') {
          $print_data .= "    <td><img src=\"".$GLOBALS['emoji_obj']->emjimg_path."/".$DDATA_A0[$a_nob]."\" border=\"0\"> ".$DDATA_A1[$a_nob]."<br><font style=\"font-size:8pt\">(No.".$a_nob.")</font><br></td>\n";
        } else {
          $print_data .= "    <td align=\"center\">-<br></td>\n";
        }
      } else {
    ...
    while ($BASEDATA = $GLOBALS['db_obj']->sql_get_data(0,$sth,'','','loop','ass','1',$GLOBALS['read_ptn'])) {
      ...
      if (isset($DDATA_D0[$BASEDATA['DoCoMo_no']])) {
        if ($DDATA_D0[$BASEDATA['DoCoMo_no']] != '') {
          $print_data .= "    <td><img src=\"".$GLOBALS['emoji_obj']->emjimg_path."/".$DDATA_D0[$BASEDATA['DoCoMo_no']]."\" border=\"0\"> ".$DDATA_D1[$BASEDATA['DoCoMo_no']]."<br><font style=\"font-size:8pt\">(No.".$BASEDATA['DoCoMo_no'].")</font><br></td>\n";
        } else {
          $print_data .= "    <td align=\"center\">-<br></td>\n";
        }
      } else {
      ...
      if (isset($DDATA_V0[$BASEDATA['SoftBank_no']])) {
        if ($DDATA_V0[$BASEDATA['SoftBank_no']] != '') {
          $print_data .= "    <td><img src=\"".$GLOBALS['emoji_obj']->emjimg_path."/".$DDATA_V0[$BASEDATA['SoftBank_no']]."\" border=\"0\"> ".$DDATA_V1[$BASEDATA['SoftBank_no']]."<br><font style=\"font-size:8pt\">(No.".$BASEDATA['SoftBank_no'].")</font><br></td>\n";
        } else {
          $print_data .= "    <td align=\"center\">-<br></td>\n";
        }
      } else {
      ...
      if (isset($DDATA_A0[$BASEDATA['au_no']])) {
        if ($DDATA_A0[$BASEDATA['au_no']] != '') {
          $print_data .= "    <td><img src=\"".$GLOBALS['emoji_obj']->emjimg_path."/".$DDATA_A0[$BASEDATA['au_no']]."\" border=\"0\"> ".$DDATA_A1[$BASEDATA['au_no']]."<br><font style=\"font-size:8pt\">(No.".$BASEDATA['au_no'].")</font><br></td>\n";
        } else {
          $print_data .= "    <td align=\"center\">-<br></td>\n";
        }
      } else {
