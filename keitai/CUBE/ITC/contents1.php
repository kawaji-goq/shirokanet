...
class Site_Contents1 extends Ab_NormalPageType{
	function Site_Contents1($dbobj) {
		$this->dbobj=$dbobj;
		$this->bname="contents1";
	}
	
	function GetSelCateList($cate_id) {
		$list=$this->dbobj->GetList("select * from contents1_cate where parents_id = ".$cate_id);
		return $list;
	}
}

class Admin_Contents1 extends Ab_AdminBasicType{
	function Admin_Contents1($dbobj) {
		$this->dbobj=$dbobj;
		$this->bname="contents1";
	}
	function GetSelCateList($cate_id) {
	...
	function CopyData($id) {
		...
		$contents1subdatalist=$this->GetSubDataList($id);
		$origindata["data_id"]=$maxid;
		for($contents1subrow=0;$contents1subdatalist[$contents1subrow];$contents1subrow++){ 
			$origindata["sub_title"][$contents1subrow]=$contents1subdatalist[$contents1subrow]["data_name"];
			$origindata["sub_comm"][$contents1subrow]=$contents1subdatalist[$contents1subrow]["data_comm"];
			$origindata["sub_num"][$contents1subrow]=$contents1subdatalist[$contents1subrow]["data_id"];
		}
