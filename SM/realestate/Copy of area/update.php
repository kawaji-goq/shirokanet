...
function AddSearch($m,$c) {
	...
	switch($m) {
		...
		case "c1":
			...
			break;
		case "c2":
			$List=$dbobj->GetList("select distinct(address2),address_h2 from zipcode2 where code ='".$c."' order by address_h2");
			break;
	}
...
function AddSearchName($p,$a) {
	...
	$dbobj->name="common";
	$dbobj->Connect();
	$List=$dbobj->GetList("select * from zipcode2 where pref like '".$p."%' and address1 like '".$a."%' order by code");
	return $List;
}

if($_REQUEST["update_area"]=="更新する") {
	$dbobj->Query("delete from re_area_master where area_id= ".$_REQUEST["area_id"]." and pref like '".$_REQUEST["p"]."'");
...
$relist=$dbobj->GetList("select address2,area_id from re_area_master where area_id=".$_REQUEST["area_id"]." and pref like '".$_REQUEST["p"]."' and address1='".$_REQUEST["a"]."'");
$alist=AddSearchName($_GET["p"],$_GET["a"]);

$limi=0;
for($i=0;$alist[$i]["code"]!=NULL;$i++) {
...
<?php if($arealist[$j][0]!=NULL) {?><input name="area_name[]" type="checkbox" id="area_name[]" value="<?php echo $arealist[$j][0];?>"<?php if(AdAry_Search($arealist[$j][0],$relist)==1){ echo " checked" ;}else if($arealist[$j][0]=="以下に掲載がない場合"){ echo  " checked" ;}?> />
<?php
		echo $arealist[$j][0];
?><?php 
								...
								<br />
								<input name="update_area" type="submit" id="update_area" value="更新する" />
