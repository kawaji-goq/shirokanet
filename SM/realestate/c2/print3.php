...
$pdf->SetDrawColor(0x89, 0x89, 0x89); 
$pdf->SetFillColor(0x89, 0x89, 0x89); 
$pdf->Rect(55,135,18,7,DF);

//4-3
$pdf->SetDrawColor(0x89, 0x89, 0x89); 
$pdf->SetFillColor(0x89, 0x89, 0x89); 
$pdf->Rect(110,135,18,7,DF);
//4-4
$pdf->SetDrawColor(0x89, 0x89, 0x89); 
$pdf->SetFillColor(0x89, 0x89, 0x89); 
$pdf->Rect(158,135,18,7,DF);
...
if($data["kakaku"]<10&&is_integer($data["kakaku"]*10)) {
	...
	$kakakudata=explode(".",$data["kakaku"]);
	$pdf->SetFont('Arial-Black','B',100);
	$pdf->SetXY(30,102);
	$pdf->Write(10,$kakakudata[0],2);
	$pdf->SetFont('Arial-Black','B',80);
	$pdf->SetXY(60,105);
	$pdf->Write(10,$kakakudata[1],2);
...
$pdf->SetFont(PGOTHIC,'',50);
$pdf->SetXY(75,22);
for($i=0;$pdf->GetStringWidth($data["bukken_mei"])>200;$i++) {
	$pdf->SetFont(PGOTHIC,'',50-$i);
	$pdf->SetXY(151,17+($i/5));
	if($i>40) {
		exit();
	}
}
...
for($i=0;$pdf->GetStringWidth($data["syumoku"]."/".$data["madori"].str_replace("ROOM","R",$data["madori_tani"]))>50;$i++) {
	$pdf->SetFont(PGOTHIC,'B',25-$i);
		$pdf->SetXY(200-$i/4,46-$i/4);
	if($i>25) {
		exit("駅名が長いためPDFの作成に失敗しました。");
	}
}

$pdf->write(0,$data["syumoku"]."/".$data["madori"].str_replace("ROOM","R",$data["madori_tani"])); 
...
if($data["shikikin"]!=NULL) { 
	$shikikin.=$data["shikikin"]."ヶ月分";
}else if($data["sikikintani"]!=NULL){
	$shikikin.=$data["sikikintani"]."万円";
}
$pdf->Write(0,$shikikin);
...
$pdf->SetTextColor(0x00, 0x00, 0x00); //前景色
if($data["reikin"]!=NULL) { 
	$reikin.=$data["reikin"]."ヶ月";
}else if($data["reikin_tani"]!=NULL){ 
	$reikin.=$data["reikin_tani"]."万円";
}
$pdf->SetXY(135,138.5);
$pdf->Write(0,$reikin);
...
if($data["kyouekihi"]!=NULL) { 
	$kyouekihi.=$data["kyouekihi"]."円";
}
...
$pdf->SetXY(10,147);
$pdf->Write(0,$chikunengetsu."築");

//駐車場
$pdf->SetXY(10,153);
$pdf->Write(0,"駐車場:".str_replace("　"," ",$data["chusyajou"]));
...
if($data["setsubi_naka1"]==1){ 
$setsubi[]="給湯";
}
if($data["setsubi_naka2"]==1){ 
$setsubi[]="冷蔵庫";
}
if($data["setsubi_naka3"]==1){ 
$setsubi[]="オール電化";
}
if($data["setsubi_naka4"]==1){ 
$setsubi[]="照明";
}
if($data["setsubi_naka5"]==1){ 
$setsubi[]="有線";
}
if($data["setsubi_naka6"]==1){ 
$setsubi[]="ケーブルテレビ";
}
if($data["setsubi_naka7"]==1){ 
$setsubi[]="インターネット対応";
}
if($data["setsubi_naka8"]==1){ 
$setsubi[]="TV";
}
if($data["setsubi_naka9"]==1){ 
$setsubi[]="居間フローリング";
}
if($data["setsubi_naka10"]==1){ 
$setsubi[]="システムキッチン";
}
if($data["setsubi_naka11"]==1){ 
$setsubi[]="室内洗濯機";
}
if($data["setsubi_naka12"]==1){ 
$setsubi[]="ウォッシュレット";
}
if($data["setsubi_naka13"]==1){ 
$setsubi[]="風呂トイレ別";
}
if($data["setsubi_naka14"]==1){ 
$setsubi[]="シャワー";
}
if($data["setsubi_naka15"]==1){ 
$setsubi[]="シャンプードレッサー";
}
if($setsubi[0]!=NULL) {
$setubitxt=implode("　",$setsubi);
$pdf->MultiCell(127,5,$setubitxt,0,2,'L');
 }
...
if($data["madorizu1"]!=NULL) {
	$pdf->Image("http://".$_REQUEST["domain"]."/tmp/bukken_data/".$data["id"]."/".str_replace("300","pop1",$data["madorizu1"]),210.5,114);
}
