...
$_REQUEST["banchichk"]=0;
	...
	if(@file_exists($_SERVER['DOCUMENT_ROOT']."/tmp/".$bname."_data/".$maxid."/".$mimagefile3["name"])&&$mimagefile3["name"]!=NULL) {
		$fdata=(pathinfo($_SERVER['DOCUMENT_ROOT']."/tmp/".$bname."_data/".$maxid."/".$mimagefile3["name"]));
		ImgMagic :: cpandconv_Size($fdata["dirname"]."/".$fdata["basename"],"o_".$fdata["basename"],$fdata["dirname"]."/",$imagesize["osusume"]["w"],$imagesize["osusume"]["h"]);
		$fdata=(pathinfo($_SERVER['DOCUMENT_ROOT']."/tmp/".$bname."_data/".$maxid."/".$mimagefile3["name"]));
...
賃貸事業用物件<font color="#000000">複製登録 </font>                                    </td>
                                </tr>
                            </table>
                        </td>
                    </tr>
