...
if($_REQUEST["pmode"]=="regist") {
	...
	if($chknum==0){
		...
		$insql="update number_of_visitors set counter = ".$_POST["counter"].",sumcounter=".$_POST["sumcount"]." where rdate = ".mktime(0,0,0,$_REQUEST["rmonth"],$_REQUEST["rday"],$_REQUEST["ryear"]);
		$chkres=$dbobj->Query($insql);
	}		
}
...
function delchk(name,id) {
	...
	if(res) {
		location.href="?PID=vister_details&pmode=delete&cate_id=<?php echo $_GET["cate_id"];?>&delid="+id;
	}
			...
			?>
										...
										<tr>
												...
												<td width="400" align="left" bgcolor="#FFFFFF">
														<input name="counter" type="text" id="counter" value="" size="20" style="ime-mode:disabled" />
												</td>
										</tr>
										<tr>
