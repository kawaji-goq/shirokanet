...
if($_REQUEST["pmode"]=="update") {
	$ad_contents1->UpdateOneCate($_POST);
	$contents1cdata=$ad_contents1->GetDetailsCate($_GET["cate_id"]);
?>
   ...
   <?php
		 if($contents1setting["cate_image_chk"]==1) {
		 ?><?php
		  }
		  ?><?php
		 if($contents1setting["cate_comm_chk"]==1) {
		 ?><?php
		  }
		  ?>
          <tr>
          ...
          <tr>
            ...
            <td align="left" valign="top" bgcolor="#FFFFFF">
              <input name="bbtm_regist" type="button" id="bbtm_regist" value="一覧へもどる" onClick="location.replace('?PID=alcontents_category')">
            </td>
          </tr>
        </table>
		<?php
		...
		<table width="700" border="0" align="center" cellpadding="3" cellspacing="1" bgcolor="#CCCCCC">
          <tr>
            <th width="100" align="left" bgcolor="#ECECEC"><strong>カテゴリ名</strong></th>
            <td width="400" align="left" bgcolor="#FFFFFF">
              <textarea name="cate_name" cols="50" id="cate_name"><?php echo $contents1cdata["cate_name"];?></textarea>
            </td>
          </tr>
           <?php
		 if($contents1setting["cate_image_chk"]==1) {
          ...
          <tr>
            ...
            <td align="left" valign="top" bgcolor="#FFFFFF">
              <select name="view_chk" id="view_chk">
                <option value="1"<?php if($contents1cdata["view_chk"]==1){echo " selected";}?>>公開する</option>
                <option value="0"<?php if($contents1cdata["view_chk"]==0){echo " selected";}?> >公開しない</option>
              </select>
            </td>
          </tr>
          <tr>
