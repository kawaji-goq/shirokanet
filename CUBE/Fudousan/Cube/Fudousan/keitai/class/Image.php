...
//$rotate			回転角度
	...
	function convert_Image($original_img,$photoname,$path,$new_long,$new_short,$rotate) {
		...
		/*
		if($orignimge_size[0]<=$new_width) {//横幅が指定より小さい時
			$imgsizelist=getimagesize($path."tmp0.".$filedata["extension"]);
			$flame_width=$new_width-$imgsizelist[0];
			$flame_height=0;
			system("convert -border ".(int)($flame_width/2)."x0 -bordercolor white ".$path."tmp0.".($filedata["extension"])." ".$filepath,$result);
			return 2;
		}
		else {//横幅が指定よりも大きいとき
			$newimg_ratio=$orignimge_size[0]/$new_width;
			$new_height=$orignimge_size[1]*$newimg_ratio;
			system("convert -geometry ".$new_width."x".$new_height." ".$path."tmp0.".($filedata["extension"])." ".$path."tmp2.".$filedata["extension"],$result);
			$result;
		}
