...
//	$encode = "EUC-JP";//文字コード
//	$host = "mail.sougou-net.jp";//ホスト
//	$username = "info@sougou-net.jp";//メールサーバ
//	$password = "itc7310!";//パスワード
//	$fromname = "総合不動産";//送信者名

	mb_language("ja");
	mb_internal_encoding("EUC-JP");
	$encode = "EUC-JP";//文字コード
	$host = "sv223.xbiz.ne.jp";//ホスト
	$username = "info@shirokanet.com";//メールサーバ
	$password = "Makiko_3613";//パスワード
	$fromname = "白兼不動産";//送信者名
	$mail->CharSet = $encode;
	...
	$mail->Port       = 587; // TCPポートを指定（tlsの場合は465や587）
    	...
    	// 送受信先設定（第二引数は省略可）
	$mail->setFrom($username, $fromname); // 送信者
	$mail->addAddress($_POST["email"], $_POST["corpname"]);   // 宛先
	$mail->addReplyTo($username, $fromname); // 返信先
	$mail->Sender = $username; // Return-path
	// 送信
	...
	$mail->Body    = $ctxt;
	$mail->send();
	$mail2->CharSet = $encode;

	//mail@shirokanet.com


	// SMTPサーバの設定
	$mail2->isSMTP();                          // SMTPの使用宣言
	$mail2->Host       = $host;   // SMTPサーバーを指定
	$mail2->SMTPAuth   = true;                 // SMTP authenticationを有効化
	$mail2->Username   = 'mail@shirokanet.com';   // SMTPサーバーのユーザ名
	$mail2->Password   = 'keW0$46i';           // SMTPサーバーのパスワード
	$mail2->SMTPSecure = 'tls';  // 暗号化を有効（tls or ssl）無効の場合はfalse
	$mail2->Port       = 587; // TCPポートを指定（tlsの場合は465や587）
	// 送受信先設定（第二引数は省略可）
	$mail2->setFrom($_POST["email"], $_POST["corpname"]); // 送信者
	$mail2->addAddress($username, $fromname);   // 宛先
	$mail2->addReplyTo($_POST["email"], $_POST["corpname"]); // 返信先
	$mail2->Sender = $username; // Return-path
	// 送信内容設定
	$mail2->Subject = $msbj;
	$mail2->Body    = $mtxt;
	$mail2->send();
	...
	//$cmail = "kawaji@itcube.jp";
	//$mmail = "yusaku.kawaji@gmail.com";
	//会社へ送信
