...
for($i=0;$cdata[$i]["area_id"];$i++) {
		...
		switch($this->type) {
			...
			case 2:
				...
				break;
			case 3:
				$sql="select * from bukken where bunrui =  ".$bunrui." and (syumoku like '%事務所%' or syumoku like '%店舗%' or syumoku like '%工場%' or syumoku like '%倉庫%' or syumoku like '%旅館%' or syumoku like '%寮%' or syumoku like '%別荘%' or syumoku like '%土地%' or syumoku like '%駐車場%' or syumoku like '%ビル%' or syumoku like '%その他%') ".$wheretxt." and del_chk <>1 order by ".$sort;
				$res=$db->Query($sql);
				$this->numrows=$db->NumRows($res);
				$sql.=$db->CreateLimitSql("",$_SESSION["lim"],$_SESSION["lim"]*($_SESSION["page"]-1));
				$rdata=$db->GetList($sql);
...
for($i=0;$cdata[$i]["area_id"];$i++) {
		...
		$cwhereary[]="( (jyusyo1 || jyusyo2) like '%".$cdata[$i]["address1"].$cdata[$i]["address2"]."%')";
			...
			switch($this->type) {
				...
				case 3:
					$sql="select * from bukken where bunrui =  ".$bunrui." and (syumoku like '%事務所%' or syumoku like '%店舗%' or syumoku like '%工場%' or syumoku like '%倉庫%' or syumoku like '%旅館%' or syumoku like '%寮%' or syumoku like '%別荘%' or syumoku like '%土地%' or syumoku like '%駐車場%' or syumoku like '%ビル%' or syumoku like '%その他%') ".$wheretxt." order by ".$sort;
					$res=$db->Query($sql);
					$this->numrows=$db->NumRows($res);
					$sql.=$db->CreateLimitSql("",$_SESSION["lim"],$_SESSION["lim"]*($_SESSION["page"]-1));
					$rdata=$db->GetList($sql);
