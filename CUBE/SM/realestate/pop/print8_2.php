...
*/		
	...
	if($bukkenlist[$x][$xy]["ekiho"]!=0&&$bukkenlist[$x][$xy]["ekiho"]<=30&&$domaindata["domain_name"]!="tabuse.itcube.ne.jp") {
		...
		$pdf->SetXY($leftbasic+1.2+$width*$num_w,$topbasic+0.5+(($height-1)*$num_h));
		...
		//ÅÌÊâ
		$pdf->SetFont(PGOTHIC,'B',9);
		$pdf->SetXY($leftbasic+1.2+$width*$num_w,$topbasic+4+(($height-1)*$num_h));
		...
		$pdf->SetXY($leftbasic+1.2+$width*$num_w,$topbasic+2+(($height-1)*$num_h));
		...
		//¹½Â¤
		if(($bukkenlist[$x][$xy]["madori"]==0||$bukkenlist[$x][$xy]["madori"]==NULL)&&$bukkenlist[$x][$xy]["senyumenseki"]!=0) {
		$syumokutxt.="/".$bukkenlist[$x][$xy]["senyumenseki"]."­Ö";
		}
		else if($bukkenlist[$x][$xy]["madori"]==0||$bukkenlist[$x][$xy]["madori"]==NULL) {
		$syumokutxt.="/".$bukkenlist[$x][$xy]["menseki"]."­Ö";
		}
		else {
		...
		//¹½Â¤
		$pdf->SetXY($leftbasic+(-1.5)+$width*$num_w,$topbasic+(42)+(($height-1)*$num_h));
		...
		//¹½Â¤
		$pdf->SetXY($leftbasic+19+$width*$num_w,$topbasic+(42)+(($height-1)*$num_h));
		$pdf->SetFont(PGOTHIC,'B',8);
		$pdf->SetTextColor(0xFF, 0xFF, 0xFF); //Á°·Ê¿§
		$pdf->Write(5,"¡¡"."Îé¶â");
		//¹½Â¤
		...
		//¹½Â¤
		$pdf->SetXY($leftbasic+37+$width*$num_w,$topbasic+(42)+(($height-1)*$num_h));
		$pdf->SetFont(PGOTHIC,'B',8);
		$pdf->SetTextColor(0xFF, 0xFF, 0xFF); //Á°·Ê¿§
		$pdf->Write(5,"¡¡"."¶¦±×ÈñÅù");
		$pdf->SetTextColor(0x00, 0x00, 0x00); //Á°·Ê¿§

		$pdf->SetXY($leftbasic+54+$width*$num_w,$topbasic+(42)+(($height-1)*$num_h));
		$sumcost=0;
		$tmpdata="";
		...
		//	$pdf->SetFont(PGOTHIC,'B',50);
			...
			//¶â³ÛÃ±°Ì
			$pdf->SetTextColor(0x00, 0x00, 0x00); //Á°·Ê¿§
			$pdf->SetFont(PGOTHIC,'B',22);
			$pdf->SetXY($leftbasic+53.5+$width*$num_w,$topbasic+33+(($height-1)*$num_h));
			$pdf->Write(5,"Ëü");
			$pdf->Write(5,"±ß");
