...
if($_REQUEST["update_re"]=="更新する") {
	...
	$imagesize["pop2"]["w"]=300;
	$imagesize["pop2"]["h"]=212;
	...
	if(@file_exists($_SERVER['DOCUMENT_ROOT']."/tmp/".$bname."_data/".$_REQUEST["bid"]."/".$imagefile1["name"])&&$imagefile1["name"]!=NULL) {
		$fdata=(pathinfo($_SERVER['DOCUMENT_ROOT']."/tmp/".$bname."_data/".$_REQUEST["bid"]."/".$imagefile1["name"]));
		ImgMagic :: cpandconv_Size($fdata["dirname"]."/".$fdata["basename"],"pop2".str_replace("png","jpg",str_replace("PNG","jpg",$fdata["basename"])),$fdata["dirname"]."/",$imagesize["pop2"]["w"],$imagesize["pop2"]["h"]);
	}
...
<TABLE width="700"  border="0" align="left" cellpadding="0" cellspacing="0" class="realestate_bgcolor1"> 
  ...
  <TR class="realestate_bgcolor2">
      <TD valign="top"><div class="helper">賃貸アパート・マンションに表示する項目を設定するには<a href="?PID=re_c1_set">こちらをクリック</a>してください。
		            <br>
      データの入力されていない項目をお客様用の物件詳細ページで表示しない場合は<a href="?PID=re_osetting">こちらをクリック</a>してください。<br>
      <font color="#FF0000">※この設定は全ての種別で共通になっております。</font></div>
      </TD>
  </TR>
  <TR class="realestate_bgcolor2">
      <TD valign="top">&nbsp;</TD>
        ...
        <table width="100%" cellpadding="1" cellspacing="1" bgcolor="#CCCCCC"> 
         ...
         <?php 
            ...
            <td bgcolor="#FFFFFF" class="font12"> 
					...
					?><tr> 
            <td width="150" valign="top" bgcolor="#EBEBEB" class="font14"> 
       		<div align="right" class="font14"><?php echo $bsetdata["bukken_hurigana_name"] ?></div>            </td> 
            <td bgcolor="#FFFFFF" class="font12"> 
					...
					?><tr> 
            <td width="150" valign="top" bgcolor="#EBEBEB" class="font14"> 
       		<div align="right" class="font14"><?php echo $bsetdata["jyusyo_name"] ?></div>            
            </td> 
...
<?php 
					...
					?>										<tr>
												...
												<td bgcolor="#FFFFFF" class="font12">
														<input name="zappi" type="text" class="noime" id="zappi" value="<?php echo $re1data["zappi"];?>" size="20">
														<span class="realestate_bgcolor3">円</span></td>
										</tr>
<?php 
					}
...
<?php 
					...
					?><input type="hidden" name="genkyou" id="genkyou" value="<?php echo $re1data["genkyou"];?>">
					<?php
					}
