...
class Site_Banner extends Ab_NormalPageType{
	function Site_Banner($dbobj) {
		$this->dbobj=$dbobj;
		$this->bname="banner";
	}
	
	function GetSelCateList($cate_id) {
		$list=$this->dbobj->GetList("select * from banner_cate where parents_id = ".$cate_id);
		return $list;
	}
}

class Admin_Banner extends Ab_AdminBasicType{
	function Admin_Banner($dbobj) {
		$this->dbobj=$dbobj;
		$this->bname="banner";
	}
	function GetSelCateList($cate_id) {
...
/***/
	...
	function UpdateOneData2($data) {
		...
		$imgobj->rpath="/tmp/".$bname."_data/".$data["data_id"]."/";
		$imagefile=$imgobj->UpImgAndResize("imagefile",200,0);	
	...
	function UpdateOneData3($data,$w,$h) {
		$db=$this->dbobj;
		$bname=$this->bname;
		...
		$imgobj->path=$_SERVER['DOCUMENT_ROOT']."/tmp/".$bname."_data/".$data["data_id"]."/";
		$imgobj->rpath="/tmp/".$bname."_data/".$data["data_id"]."/";
		$imagefile=$imgobj->UpImgAndResize("imagefile",$w,$h);	
...
/**/	
	...
	function CopyData($id) {
		...
		$bannersubdatalist=$this->GetSubDataList($id);
		$origindata["data_id"]=$maxid;
		for($bannersubrow=0;$bannersubdatalist[$bannersubrow];$bannersubrow++){ 
			$origindata["sub_title"][$bannersubrow]=$bannersubdatalist[$bannersubrow]["data_name"];
			$origindata["sub_comm"][$bannersubrow]=$bannersubdatalist[$bannersubrow]["data_comm"];
			$origindata["sub_num"][$bannersubrow]=$bannersubdatalist[$bannersubrow]["data_id"];
		}
