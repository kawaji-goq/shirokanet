...
class Site_Info1 extends Ab_NormalPageType{
	function Site_Info1($dbobj) {
		$this->dbobj=$dbobj;
		$this->bname="info1";
	}
	
	function GetSelCateList($cate_id) {
		$list=$this->dbobj->GetList("select * from info1_cate where parents_id = ".$cate_id);
		return $list;
	}
}

class Admin_Info1 extends Ab_AdminBasicType{
	function Admin_Info1($dbobj) {
		$this->dbobj=$dbobj;
		$this->bname="info1";
	}
	function GetSelCateList($cate_id) {
	...
	function UpdateOneData($data) {
		...
		$imgobj0=new Upload();
		$imgobj0->path=$_SERVER['DOCUMENT_ROOT']."/tmp/".$bname."_data/";
		$imgobj0->rpath="/tmp/".$bname."_data/".$data["data_id"]."/";
		$imagefile0=$imgobj0->UpImgAndResize("maker_logo",$setdata["makerlogo_w"],$setdata["makerlogo_h"]);	
		...
		if($imagefile3["filepath"]!=NULL) {
			...
			$ftp->MkDir("tmp",$bname."_data");
			$ftp->MkDir("tmp/".$bname."_data",$data["data_id"]);
			$ftp->UpData("tmp/".$bname."_data/".$data["data_id"]."/",$imagefile3["name"],$_SERVER['DOCUMENT_ROOT']."/tmp/".$bname."_data/".$imagefile3["name"],"b");
		}
		else if($data["delimage3"]==1){
			if($setdata["detailsimg3_defalt"]!=NULL){
				$field[]="data_image3";
		...
		if($imagefile0["filepath"]!=NULL) {
			...
			$ftp->MkDir("tmp",$bname."_data");
			$ftp->MkDir("tmp/".$bname."_data",$data["data_id"]);
			$ftp->UpData("tmp/".$bname."_data/".$data["data_id"]."/",$imagefile0["name"],$_SERVER['DOCUMENT_ROOT']."/tmp/".$bname."_data/".$imagefile0["name"],"b");
		}
		else if($data["delmakerlogo"]==1){
				$field[]="maker_logo";
				$tdata[]="''";
	...
	function CopyData($id) {
		...
		$info1subdatalist=$this->GetSubDataList($id);
		$origindata["data_id"]=$maxid;
		for($info1subrow=0;$info1subdatalist[$info1subrow];$info1subrow++){ 
			$origindata["sub_title"][$info1subrow]=$info1subdatalist[$info1subrow]["data_name"];
			$origindata["sub_comm"][$info1subrow]=$info1subdatalist[$info1subrow]["data_comm"];
			$origindata["sub_num"][$info1subrow]=$info1subdatalist[$info1subrow]["data_id"];
		}
