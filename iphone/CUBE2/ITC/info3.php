...
class Site_Info3 extends Ab_NormalPageType{
	function Site_Info3($dbobj) {
		$this->dbobj=$dbobj;
		$this->bname="info3";
	}
	
	function GetSelCateList($cate_id) {
		$list=$this->dbobj->GetList("select * from info3_cate where parents_id = ".$cate_id);
		return $list;
	}
}

class Admin_Info3 extends Ab_AdminBasicType{
	function Admin_Info3($dbobj) {
		$this->dbobj=$dbobj;
		$this->bname="info3";
	}
	function GetSelCateList($cate_id) {
	...
	function AdditionData($data) {
		...
		if($data["new_data_name"]!=NULL||$data["new_data_comm"]!=NULL) {
			...
			@system("chmod -Rf 0777 ".$_SERVER['DOCUMENT_ROOT']."/tmp/*");
			...
			$imgobj0=new Upload();
			$imgobj0->path=$_SERVER['DOCUMENT_ROOT']."/tmp/".$bname."_data/".$maxid."/";
		...
		if($domaindata["dbtype"]=="mysql") {
			...
			if($imagefile0["filepath"]!=NULL) {
				...
				$ftp->MkDir("tmp",$bname."_data");
				$ftp->MkDir("tmp/".$bname."_data",$maxid);
				$ftp->UpData("tmp/".$bname."_data/".$maxid."/",$imagefile0["name"],$_SERVER['DOCUMENT_ROOT']."/tmp/".$bname."_data/".$maxid."/".$imagefile0["name"],"b");
			}
	...
	if($_SERVER['HTTP_HOST']=="siteadmin.itcube.ne.jp"){
		...
		if($imagefile["filepath"]!=NULL) {
			...
			$ftp->UpData("tmp/".$bname."_data/".$data["data_id"]."/",$imagefile["name"],"../tmp/".$_SESSION["DomainData"]["dbname"]."/".$bname."_data/".$data["data_id"]."/".$imagefile["name"],"b");
			}
		}
		...
		if($imagefile2["filepath"]!=NULL) {
			$field[]="data_image2";
			$tdata[]="'".$imagefile2["filepath"]."'";
			if($_SERVER['HTTP_HOST']=="siteadmin.itcube.ne.jp"){$ftp=new Cube_FTP();
			$ftp->Connect($_SESSION["DomainData"]["id"],$_SESSION["DomainData"]["pass"]);
			@chmod($imagefile2["filepath"],0777);
			$ftp->MkDir("tmp",$bname."_data");
			$ftp->MkDir("tmp/".$bname."_data",$data["data_id"]);
			$ftp->UpData("tmp/".$bname."_data/".$data["data_id"]."/",$imagefile2["name"],"../tmp/".$_SESSION["DomainData"]["dbname"]."/".$bname."_data/".$data["data_id"]."/".$imagefile2["name"],"b");
			}
		}
		...
		if($imagefile3["filepath"]!=NULL) {
			...
			$ftp->MkDir("tmp",$bname."_data");
			$ftp->MkDir("tmp/".$bname."_data",$data["data_id"]);
			$ftp->UpData("tmp/".$bname."_data/".$data["data_id"]."/",$imagefile3["name"],"../tmp/".$_SESSION["DomainData"]["dbname"]."/".$bname."_data/".$data["data_id"]."/".$imagefile3["name"],"b");
			}
		}
		...
		if($imagefile0["filepath"]!=NULL) {
			...
			$ftp->MkDir("tmp",$bname."_data");
			$ftp->MkDir("tmp/".$bname."_data",$data["data_id"]);
			$ftp->UpData("tmp/".$bname."_data/".$data["data_id"]."/",$imagefile0["name"],"../tmp/".$_SESSION["DomainData"]["dbname"]."/".$bname."_data/".$data["data_id"]."/".$imagefile0["name"],"b");
			}
}
...
//		echo $setdata["detailsimg_w"];
	...
	function CopyData($id) {
		...
		@mkdir($_SERVER['DOCUMENT_ROOT']."/tmp/".$bname."_data/".$maxid);		
		@exec("cp -Rf ".$_SERVER['DOCUMENT_ROOT']."/tmp/".$bname."_data/".$id."/* ".$_SERVER['DOCUMENT_ROOT']."/tmp/".$bname."_data/".$maxid."/");
		...
		$field[]="data_image";
		$tdata[]="'".str_replace("/".$id."/","/".$maxid."/",$origindata["data_image"])."'";
		...
		$info3subdatalist=$this->GetSubDataList($id);
		$origindata["data_id"]=$maxid;
		for($info3subrow=0;$info3subdatalist[$info3subrow];$info3subrow++){ 
			$origindata["sub_title"][$info3subrow]=$info3subdatalist[$info3subrow]["data_name"];
			$origindata["sub_comm"][$info3subrow]=$info3subdatalist[$info3subrow]["data_comm"];
			$origindata["sub_num"][$info3subrow]=$info3subdatalist[$info3subrow]["data_id"];
		}
