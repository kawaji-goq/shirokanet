...
function Normal($pdata,$tenpodata)
{
	...
	"-------------------------------------------------------\n".
	"会社名・お名前　　　　　：".$pdata["corpname"]."\n".
	"メールアドレス　：".$pdata["email"]."\n".
	"お電話番号　　　：".$pdata["telnumber"]."\n".
	"FAX番号　　　　 ：".$pdata["faxnumber"]."\n".
	"〒　 ：".$pdata["zipcode"]."\n".
	"所在地　 ：".$pdata["address"]."\n".
	"-------------------------------------------------------\n".
	"ご意見・お問い合わせ等\n".
	"-------------------------------------------------------\n".
	$pdata["comment"].$pdata["psbukken"];//会社宛本文
	
	$mmail=$pdata["email"];//返信用メールアドレス
	$msbj=$tenpodata["name"]."に送信しました。";//返信用タイトル
	...
	$encode = "EUC-JP";//文字コード
	$host = "mail.miyamoto-fu.jp";//ホスト
	$username = "info@miyamoto-fu.jp";//メールサーバ
	$password = "O4y7jd&5";//パスワード
	$fromname = "宮本不動産";//送信者名
