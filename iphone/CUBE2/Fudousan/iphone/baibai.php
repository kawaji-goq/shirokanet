			...
			<table border="0" cellpadding="0" cellspacing="0" align="center" class="bukkenlist_on" <?php if($re1rows==0){?>style="margin-top:5px;"<?php }?>>
				<tr>
					<td rowspan="4" width="40%" class="bukkenlist_img"<?php if($re1rows==0){?> style="-webkit-border-top-left-radius: 3px;"<?php } ?>>
						<a href="/iphone/baibai_d.php?bid=<?php echo $re1data[$re1rows]["id"]; ?>">
<?php
...
?>
						</a><br><br><input type="button" class="baibai_link" value="詳細" onClick="location.href='/iphone/baibai_d.php?bid=<?php echo $re1data[$re1rows]["id"]; ?>'">
					</td>
...
<?php
	...
	{
		echo "<strong><font color=#FF52C8>".number_format($re1data[$re1rows]["kakaku"],1)."万円</font></strong>";
	}
	else
	{
		echo "-";
...
{
?>
				<a href="../iphone_back_up/?cid=<?php echo $_SESSION["cid"];?>&page=<?php echo $_SESSION["page"]-1;?>">＜ 前の10件</a>
<?php
}
?>
			</div>
...
?>
		...
		<td width="100" nowrap="nowrap">
			<div align="left">
				...
				<a href="../iphone_back_up/?cid=<?php echo $_SESSION["cid"];?>&page=<?php echo $_SESSION["page"]+1;?>">次の10件 ＞</a>
				<?php } ?>
			</div>
		</td>
	</tr>
